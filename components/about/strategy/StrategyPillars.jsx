    ...
    <div className="container mx-auto px-4">
      <div className="max-w-3xl mb-16 animate-in fade-in slide-in-from-left duration-700">
        <h2 className="text-4xl lg:text-5xl font-black text-primary leading-tight">
          {data.title} <span className="text-secondary">{data.highlight}</span>
        </h2>
      </div>
      <div className="grid md:grid-cols-3 gap-8">
        {data.items.map((pillar, i) => {
          ...
          return (
            <div
              key={i}
              className="bg-white p-10 rounded-[2.5rem] shadow-sm hover:shadow-2xl transition-all duration-500 border border-slate-100 group"
            >

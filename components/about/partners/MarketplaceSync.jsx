...
export const MarketplaceSync = ({ data: marketplace }) => {
  return (
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            <Badge
              variant="outline"
              className="border-secondary text-secondary bg-secondary/5 px-4 py-1"
            >
              {marketplace.badge}
            </Badge>
            <h2 className="text-4xl md:text-5xl lg:text-6xl font-black tracking-tighter text-foreground leading-tight">
              {marketplace.title} <br />{" "}
              <span className="text-primary">{marketplace.titleHighlight}</span>
            </h2>
          ...
          <div className="grid sm:grid-cols-2 gap-6 relative">
            {marketplace.platforms.map((platform, idx) => (
              <div
                ...
                whileHover={{ y: -12 }}
                className="group relative h-[320px] bg-primary rounded-[2.5rem] p-8 flex flex-col justify-between overflow-hidden shadow-2xl shadow-primary/20"
              >
                ...
                <div className="relative z-10 mt-6">
                  <h3 className="text-white text-xl font-bold mb-2 flex items-center justify-between">
                    {platform.label}{" "}
                    <ArrowUpRight className="w-5 h-5 text-secondary group-hover:translate-x-1 transition-transform" />
                  </h3>

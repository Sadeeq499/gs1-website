    ...
    <section className="py-24 bg-background">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} className="space-y-8">
          ...
          <div className="grid sm:grid-cols-2 gap-6 relative">
            {marketplace.platforms.map((platform, idx) => (
              <div key={platform.name} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: platform.delay }} whileHover={{ y: -12 }} className="group relative h-[320px] bg-primary rounded-[2.5rem] p-8 flex flex-col justify-between overflow-hidden shadow-2xl shadow-primary/20">
